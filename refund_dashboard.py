...
# ---------------------- 登录模块（新增读取is_admin超级管理员标识，修复SQL拼接崩溃） ----------------------
def login_page():
    st.subheader("账号登录")
...
def dashboard_main():
    ...
    with col4:
        shop_count = df_data["店铺"].nunique() if not df_data.empty else 0
    ...
    if not df_data.empty:
        day_trend = df_data.groupby(df_data["退款时间"].dt.date)[qty_col].sum().reset_index()
